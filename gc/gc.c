...
/* NOTE: I modified this library with mimalloc for faster performance - Calloc */
#include "mimalloc.h"
...
{
    Allocation* a = (Allocation*) mi_malloc(sizeof(Allocation));
    a->ptr = ptr;
    a->size = size;
    a->tag = GC_TAG_NONE;
    a->dtor = dtor;
...
static AllocationMap* gc_allocation_map_new(size_t min_capacity,
        ...
        double upsize_factor)
{
    AllocationMap* am = (AllocationMap*) mi_malloc(sizeof(AllocationMap));
    ...
    am->allocs = (Allocation**) mi_calloc(am->capacity, sizeof(Allocation*));
    am->size = 0;
...
{
    if (!count) return mi_malloc(size);
